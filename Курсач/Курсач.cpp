...
{
	int item=0, ch;
	string MenuItem[3]={"Запустити програму на виконання","Посібник користувача           ","Вийти з програми               "};
	while(true)
	{
		system("cls");
		textcolor(LightCyan);
...
void work()
{
	...
	while(true)
	{
		...
		textcolor(White);
		gotoxy(2,4);
		stroka(15,"У прямокутнику введіть розмірність матриці коефіцієнтів та натисніть ");
	...
	for(int i=0; i<n; i++)
		for(int j=0; j<n; j++)
			while(true)
			{
				...
				textcolor(White);
				gotoxy(2,4);
				stroka(15,"У прямокутнику введіть коефіцієнт A");
				textcolor(LightRed);
				stroka(15,"[");
				textcolor(White);
				printf("%d",i+1);
	...
	for(int i=0; i<n; i++)
		while(true)
		{
			...
			textcolor(White);
			gotoxy(2,4);
			stroka(15,"У прямокутнику введіть вільний член B");
			textcolor(LightRed);
			stroka(15,"[");
			textcolor(White);
			printf("%d",i+1);
